...
catalog = Catalog(app).load(app.config["TRACKS_CATALOG"], app.config["TOP_TRACKS_CATALOG"],
                            app.config["TRACKS_WITH_DIVERSE_RECS_CATALOG"], app.config["CONTEXT_EMBEDDINGS"],
                            app.config["TRACK_EMBEDDINGS"], app.config["USER_EMBEDDINGS"])
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        if treatment == Treatment.T1:
            recommender = HwRecommender(tracks_redis.connection, catalog.context_embeddings,
                                        catalog.track_embeddings, catalog.user_embeddings, 25)
        else:
